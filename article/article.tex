...
%% This article is connected to source code "src/CLI/main.cpp".
...
\begin{frontmatter}
	...
	\title{Scalable Exact Pump Scheduling in Water Distribution Networks via Parallel Branch-and-Bound with Snapshot Persistence}

	\author[ufc]{Michael Souza\corref{cor1}}
	\ead{michael@ufc.br}

	\author[ufc]{Jéssica Gomes}

	\author[ufc]{Albert Einstein Fernandes Muritiba}
	\ead{einstein@ufc.br}

	\author[ufc]{Ascânio Dias Araújo}
	\ead{ascanio@fisica.ufc.br}

	\cortext[cor1]{Corresponding author}

	\affiliation[ufc]{organization={Universidade Federal do Ceará},
		city={Fortaleza},
		state={CE},
		country={Brazil}}

	\begin{abstract}
		Pump scheduling in water distribution networks presents a challenging combinatorial optimization problem: minimizing energy costs while satisfying complex hydraulic constraints. Meta-heuristic approaches dominate the literature due to their flexibility, yet they cannot guarantee global optimality. Exact methods such as Branch-and-Bound (B\&B) provide such guarantees but have historically been computationally prohibitive. We present a scalable B\&B framework that extends Costa et al.'s foundational work through three key innovations: MPI-based parallelism for distributed computation, hydraulic snapshot persistence enabling $O(1)$ state restoration during backtracking, and strict multi-criteria pruning within a depth-first search strategy. Integrated with the EPANET~3 hydraulic engine \cite{Rossman2020}, our solver reduces optimization time by one to two orders of magnitude while improving upon the best-known solutions from meta-heuristics. On the AnyTown Modified benchmark, speedups of 29--115$\times$ over sequential implementations were observed, with cost reductions of up to 3.7\% compared to prior methods. These advances make exact pump scheduling practical for operational planning in water networks of comparable scale.
	\end{abstract}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.7\columnwidth]{figures/two_level_diagram.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\columnwidth]{figures/tree_decomposition.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\columnwidth]{figures/anytown_network.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.9\columnwidth]{figures/anytown_energy_cost.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\linewidth]{figures/tank_levels_24h.png}
...
\begin{figure*}[htbp]
	\centering
	\includegraphics[width=\textwidth]{figures/comparison_table_a1.png}
...
\begin{figure*}[htbp]
	\centering
	\includegraphics[width=\textwidth]{figures/comparison_table_a2.png}
...
\begin{figure*}[htbp]
	\centering
	\includegraphics[width=\textwidth]{figures/comparison_table_a3.png}
