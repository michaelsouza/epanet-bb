...
\begin{abstract}
  Pump scheduling in water distribution networks is a challenging mixed-integer nonlinear program (MINLP) that seeks to minimize energy costs while satisfying complex hydraulic constraints. Meta-heuristic approaches dominate the literature due to their flexibility but cannot guarantee global optimality. Exact methods such as Branch-and-Bound (B\&B) provide such guarantees but have historically been computationally prohibitive for realistic networks. This paper presents a scalable parallel B\&B framework that combines depth-first search with cooperative pruning, decomposition of the search tree into MPI tasks, hydraulic snapshot persistence for constant-time state restoration during backtracking, and a Least Used Selection (LUS) strategy that dynamically balances actuation budgets across pumps to preserve feasibility deeper in the search tree. Candidate schedules are evaluated through hydraulic simulation in EPANET~3, allowing the method to enforce pressure and storage constraints while preserving exact optimality guarantees. The framework was tested on a benchmark dataset over a 24-hour planning horizon with multiple actuation limits. The proposed solver reduced computation time by one to two orders of magnitude relative to prior sequential exact methods, achieving speedups of 29 to 115 times. It also improved the best previously reported heuristic solutions in the least constrained case. These results show that exact pump scheduling is now feasible for day-ahead operational planning on realistic study networks. Beyond direct operational use, the method can serve as a benchmark for assessing heuristic methods and quantifying the gap between fast approximate schedules and the true optimum for a given hydraulic model. The source code is publicly available.
\end{abstract}

\section{Practical Applications}
Operating pumps is one of the largest daily energy expenses for water utilities, so better schedules can translate directly into lower costs. In many systems, schedules are still based on operator experience or search methods that provide good answers but cannot confirm whether a better solution exists. This study shows that 24-hour pump schedules can now be optimized exactly within time frames useful for practice.  For a standard benchmark system with three pumps and three storage tanks, the method reduced energy costs by up to 3.7\% compared with the best published heuristic results and cut solution times by as much as 115 times compared with earlier exact approaches. Depending on operating constraints, run times dropped from days to hours, or from hours to minutes, on a standard Linux server. Because the method is built on EPANET~3, utilities and consultants can apply it using existing network models. In practice, this supports day-ahead pump operation decisions with lower energy cost, compliance with hydraulic limits, and confidence in solution quality. It also provides a benchmark for testing and calibrating heuristic scheduling tools by showing how close faster approximate solutions are to the best solution for the modeled system.
...
The computational tractability of exact methods for realistic pump scheduling was an open question until \citeN{Costa2016} demonstrated that Branch-and-Bound (B\&B) \cite{Land1960} could solve the problem for small-to-medium networks when appropriately constrained. Their key insight, which runs counter to typical assumptions, was that tighter operational constraints actually improve solver performance by enabling effective pruning of infeasible branches. In particular, limits on pump actuation frequency proved especially effective. However, their sequential implementation required hours or days of computation, limiting practical applicability. Recent efforts have also focused on developing tighter mathematical relaxations to improve B\&B performance. For instance, \citeN{Bonvin2021} proposed a tailored Mixed-Integer Linear Programming (MILP) relaxation using Outer Approximation (OA) to handle the non-convex hydraulic laws, providing an exact method applicable to both fixed and variable-speed pumps. While they focus on mathematical tightness to prune the search space, scalability remains a primary bottleneck for exact methods \cite{Reis2023,MalaJetmarova2018}.
...
We present a distributed-memory parallel Branch-and-Bound (B\&B) algorithm integrated with the EPANET~3 hydraulic engine. We adopt the two-level schedule representation proposed by \citeN{Costa2016}, minimizing the branching factor, but enhance it with a Least Used Selection (LUS) strategy. To address scalability limitations of prior exact methods, we introduce three architectural innovations: cost-based pruning (which was not utilized in \citeA{Costa2016}), MPI-based static load balancing with cooperative pruning, and a hydraulic snapshot mechanism for $O(1)$ state restoration.

\subsection{Two-Level Schedule Representation}

To mitigate the $2^{N_p \times T}$ combinatorial explosion, we adopt the variable separation strategy of \citeN{Costa2016}. A high-level decision variable $y_h \in \{0, 1, \dots, N_p\}$ represents the number of active pumps at hour $h$. The low-level binary configuration $\mathbf{x}_h = \{x_{1,h}, \dots, x_{N_p,h}\}$ is derived from $y_h$ via a deterministic greedy mapping:
\[
	\mathcal{M}: (y_h, \mathbf{x}_{h-1}, \mathcal{R}) \to \mathbf{x}_h
\]
where $\mathcal{R}$ denotes the remaining actuation budget for each pump. Fig.~\ref{fig:two_level} illustrates this representation: the high-level schedule $y_h$ specifies that 2, 3, 3, 2, 1, and 0 pumps should be active over six hours, which the mapping $\mathcal{M}$ converts into explicit binary pump states $x_{j,h}$.
...
We decompose the search tree into independent sub-problems to exploit distributed-memory parallelism. Each \emph{task} is defined by a fixed prefix of the high-level schedule, $\mathbf{y}_{1:L} = \{y_1, \dots, y_L\}$, where $L$ denotes the decomposition level (typically $L=8$). This yields $(N_p + 1)^L$ total tasks. Fig.~\ref{fig:tree_decomposition} illustrates this decomposition: prefixes are enumerated at level $L$, and each resulting subtree is assigned to an MPI rank for independent exploration.
...
To evaluate solver performance, we employ the AnyTown Modified network, a well-established benchmark in pump scheduling research. This network was originally proposed by \citeN{Walski1987} and subsequently modified by \citeN{Rao2006} to increase optimization complexity. The modified network comprises 44 pipes, 25 nodes, and three storage tanks located at nodes 65, 165, and 265. Water is supplied from a source at node 10 through three identical fixed-speed pumps operating in parallel (111, 222, and 333). All demand nodes follow the same temporal pattern, scaled by individual demand multipliers. Storage tanks operate within level bounds of 66.53 m (minimum) and 71.53 m (maximum). Fig.~\ref{fig:anytown_network} shows the network topology used in our experiments, and Fig.~\ref{fig:anytown_energy_cost} shows the hourly energy price pattern used for pump operating costs. This benchmark is particularly valuable for benchmarking because it captures the essential features of real-world systems (multiple pumps, storage facilities, and time-varying demands) while remaining tractable for comprehensive optimization studies.
...
We compare our framework with the sequential B\&B of \citeN{Costa2016}, the Genetic Algorithm of \citeN{Cimorelli2020}, and the Digital Harmony Search (DHS) of \citeN{Paola2024} on AnyTown Modified. All reference solutions were re-evaluated using EPANET~3.0 to ensure consistency; each remained hydraulically feasible with costs matching original reports within 0.1\% (differences attributable to minor variations between EPANET~2 and EPANET~3 hydraulic engines).

Figs.~\ref{fig:comparison_a1}--\ref{fig:comparison_a3} present detailed pump schedules for all methods at each actuation limit. These visualizations show that our schedules differ from prior approaches in pump timing and activation sequences while achieving superior cost performance.
...
The source code and benchmark data required to reproduce the results presented in
this paper are available at \url{https://github.com/michaelsouza/epanet-bb}.

\section*{Acknowledgments}

This research was partially funded by the Brazilian research agencies FAPESP
(Grant Nos.\ 2013/07375-0, 2023/08706-1, and 2024/00923-6) and CNPq
