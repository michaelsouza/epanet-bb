...
Operating pumps is one of the largest daily energy expenses for water utilities, so
better schedules can translate directly into lower costs. In many systems, schedules
are still based on operator experience or search methods that provide good answers
but cannot confirm whether a better solution exists. This study shows that 24-hour
pump schedules can now be optimized exactly within time frames useful for practice.
For a standard benchmark system with three pumps and three storage tanks, the method
reduced energy costs by up to 3.7\% compared with the best published heuristic
results and cut solution times by as much as 115 times compared with earlier exact
approaches. Depending on operating constraints, run times dropped from days to hours,
or from hours to minutes, on a standard Linux server. Because the method is built on
EPANET~3, utilities and consultants can apply it using existing network models. In
practice, this supports day-ahead pump operation decisions with lower energy cost,
compliance with hydraulic limits, and confidence in solution quality. It also
provides a benchmark for testing and calibrating heuristic scheduling tools by
showing how close faster approximate solutions are to the best solution for the
modeled system.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.5\columnwidth]{figures/Figure_1_two_level_diagram.png}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\columnwidth]{figures/Figure_4_anytown_energy_cost.png}
