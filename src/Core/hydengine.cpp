...
static const string s_Unbalanced = "  WARNING - network is unbalanced. Flows "
                                   "and pressures may not be correct.";
...
void HydEngine::open(Network *nw) {
  ...
  if (engineState != HydEngine::CLOSED)
    close();
  network = nw;
  ...
  matrixSolver = MatrixSolver::factory(network->option(Options::MATRIX_SOLVER),
                                       network->msgLog);
  ...
  hydSolver = HydSolver::factory(network->option(Options::HYD_SOLVER), network,
                                 matrixSolver);
...
void HydEngine::initMatrixSolver() {
  ...
  try {
    ...
    matrixSolver->init(nodeCount, linkCount, (int *)&node1[0],
                       (int *)&node2[0]);
  } catch (...) {
    throw;
  }
...
int HydEngine::resolvePressureDeficiency(int &trials) {
  ...
  for (Node *node : network->nodes) {
    if (node->type() == Node::JUNCTION && node->fixedGrade) {
      ...
      if (node->actualDemand < node->fullDemand)
        count1++;
...
void HydEngine::reportDiagnostics(int statusCode, int trials) {
  ...
  if (network->option(Options::REPORT_STATUS)) {
    ...
    case HydSolver::FAILED_NO_CONVERGENCE:
      if (halted)
        network->msgLog << s_UnbalancedHalted;
      else
        network->msgLog << s_Unbalanced;
      break;
...
int HydEngine::timeToCloseTank(int tstep) {
  ...
  for (Node *node : network->nodes) {
    ...
    if (node->type() == Node::TANK) {
      ...
      if (t <= 0)
        t = tank->timeToVolume(tank->maxVolume);
...
int HydEngine::timeToActivateControl(int tstep) {
  ...
  if (activated)
    timeStepReason = "  (control activated)";
  return tstep;
}
...
void HydEngine::updateEnergyUsage() {
  ...
  int dt = hydStep;
  if (network->option(Options::TOTAL_DURATION) == 0)
    dt = 86400; // 1 day
  if (dt == 0)
    return;
