...
void Tank::convertUnits(Network *nw) {
  ...
  if (bulkCoeff == MISSING)
    bulkCoeff = nw->option(Options::BULK_COEFF);
}
...
void Tank::initialize(Network *nw) {
  ...
  updateArea();
  if (volCurve)
    minVolume = findVolume(minHead);
  else if (minVolume == 0.0)
    minVolume = (minHead - elev) * area;
