...
double PumpEnergy::updateEnergyUsage(Pump *pump, Network *network, int dt) {
  ...
  if (pump->status == Link::LINK_CLOSED || pump->speed == 0 ||
      pump->flow < NO_FLOW)
    return 0.0;
  ...
  kwHrsPerCFS =
      ((kwHrsPerCFS * hrsOnLine) + (kw / pump->flow * hrs)) / totalHrs;

  totalCost =
      ((totalCost * hrsOnLine) + (kw * findCostFactor(pump, network) * hrs)) /
      totalHrs;
  ...
  // Calculate the total cost using the adjusted power
  adjustedTotalCost += kw * hrs * findCostFactor(pump, network);
...
double PumpEnergy::findCostFactor(Pump *pump, Network *network) {
  ...
  if (pump->costPattern)
    costFactor = pump->costPattern->currentFactor();
