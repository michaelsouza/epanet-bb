...
ReportWriter::ReportWriter(ofstream &ofs, Network *nw)
    : sout(ofs), network(nw) {}
...
void ReportWriter::writeSummary(string inpFileName) {
  ...
  if (!network->option(Options::REPORT_SUMMARY))
    return;
  ...
  sout << "\n  Head Loss Model ............... "
       << network->option(Options::HEADLOSS_MODEL);
  sout << "\n  Leakage Model ................. "
       << network->option(Options::LEAKAGE_MODEL);
  sout << "\n  Demand Model .................. "
       << network->option(Options::DEMAND_MODEL);
  sout << "\n  Demand Multiplier ............. "
       << network->option(Options::DEMAND_MULTIPLIER);
  sout << "\n  Number of Emitters ............ " << nEmitters;
  sout << "\n  Head Tolerance ................ "
       << network->option(Options::HEAD_TOLERANCE);
  sout << "\n  Flow Tolerance ................ "
       << network->option(Options::FLOW_TOLERANCE);
  sout << "\n  Flow Change Limit ............. "
       << network->option(Options::FLOW_CHANGE_LIMIT);

  sout << "\n  Quality Model ................. "
       << network->option(Options::QUAL_MODEL);
  ...
  if (network->option(Options::QUAL_TYPE) == Options::CHEM)
    sout << "\n  Quality Constituent ........... "
         << network->option(Options::QUAL_NAME);
  if (network->option(Options::QUAL_TYPE) != Options::NOQUAL)
    sout << "\n  Number of Sources ............. " << nSources;

  sout << "\n  Hydraulic Time Step ........... "
       << network->option(Options::HYD_STEP) / 60 << " minutes";
  if (network->option(Options::QUAL_TYPE) != Options::NOQUAL)
    sout << "\n  Quality Time Step ............. "
         << network->option(Options::QUAL_STEP) << " seconds";
  sout << "\n  Report Time Step .............. "
       << network->option(Options::REPORT_STEP) / 60 << " minutes";
  sout << "\n  Total Duration ................ "
       << network->option(Options::TOTAL_DURATION) / 3600 << " hours";
  sout << "\n";
}

//-----------------------------------------------------------------------------

//  Access the results saved to a binary file to write a formatted report a text
//  file.
...
int ReportWriter::writeReport(string inpFileName, OutputFile *outFile) {
  ...
  if (rptFileName2.length() > 0) {
    ...
    if (!sout.is_open())
      return FileError::CANNOT_OPEN_REPORT_FILE;
    usingRptFile2 = true;
  ...
  // ... otherwise write report to the primary report file
    ...
    if (!sout.is_open())
      return FileError::CANNOT_WRITE_TO_REPORT_FILE;
...
//  Write the energy usage results saved in a binary output file to the report
//  file.

void ReportWriter::writeEnergyResults(OutputFile *outFile) {
  if (!network->option(Options::REPORT_ENERGY))
    return;
  ...
  int pumpIndex;
  float totalAvgRate = 0.0;
  double H = outFile->timePeriodCount > 0
                  ? (double)outFile->timePeriodCount * (double)outFile->reportStep / 3600.0
                  : 24.0;
  double totalAdjPerDay = 0.0;
  for (int p = 0; p < nPumps; p++) {
    ...
    totalAvgRate += outFile->pumpResults[5];
    double p_online = outFile->pumpResults[0] / 100.0; // fraction online
    double adjPerDay = outFile->pumpResults[5] * p_online * H / 100.0;
    totalAdjPerDay += adjPerDay;
  }
  sout << left;
  string line(96, '-');
  sout << "  " << line << endl;
  ...
  sout << left << endl;
  // Summaries: total avg rate and proper energy totals per day
  sout << setw(86) << "  Total Avg Cost (cents/h):";
  sout << right;
  writeNumber(totalAvgRate, 12, 2);
  sout << left << endl;
  sout << setw(86) << "  Energy Total (/day, $):";
  sout << right;
  writeNumber(totalAdjPerDay, 12, 2);
  sout << left << endl;
  sout << setw(86) << "  Total (Energy + Demand) (/day, $):";
  sout << right;
  writeNumber(totalAdjPerDay + demandCharge, 12, 2);
  sout << left << endl;
}
...
void ReportWriter::writeEnergyHeader() {
  ...
  sout << setw(12) << "Avg." << setw(12) << "Peak" << setw(12) << "Avg cost"
       << endl;
  ...
  string volume;
  if (network->option(Options::UNIT_SYSTEM) == Options::US)
    volume = "Mgal";
  else
    volume = "m3";
  sout << setw(12) << volume;
  sout << setw(12) << "Kwatts" << setw(12) << "Kwatts" << setw(12) << "cents/h"
       << endl;
...
//  Write the energy usage results for a specific pump to the formatted report
//  file.
...
void ReportWriter::writePumpResults(Link *link, float *x) {
  ...
  for (int i = 0; i < NumPumpVars; i++)
    writeNumber(x[i], 12, 2);
  sout << endl;
}
...
void ReportWriter::writeNodeResults(Node *node, float *x) {
  ...
  for (int i = 0; i < NumNodeVars - 1; i++)
    writeNumber(x[i], width, precis);
  if (network->option(Options::QUAL_TYPE) != Options::NOQUAL) {
...
void ReportWriter::writeNodeHeader() {
  ...
  sout << setw(12) << network->getUnits(Units::FLOW);
  if (hasQual)
    sout << setw(12) << network->option(Options::QUAL_UNITS_NAME);
  sout << endl;
...
void ReportWriter::writeLinkHeader() {
  ...
  sout << "  " << s1 << endl;
  sout << setw(26) << " ";
  sout << "   Flow Rate     Leakage    Velocity   Head Loss      Status"
       << endl;
