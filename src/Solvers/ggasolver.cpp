...
void GGASolver::setConvergenceLimits() {
  ...
  headErrLimit =
      network->option(Options::HEAD_TOLERANCE) / network->ucf(Units::LENGTH);
  flowErrLimit =
      network->option(Options::FLOW_TOLERANCE) / network->ucf(Units::FLOW);

  // ... limit on largest flow change
  flowChangeLimit =
      network->option(Options::FLOW_CHANGE_LIMIT) / network->ucf(Units::FLOW);

  // ... use a default head error limit if need be
  if (flowRatioLimit == 0.0 && headErrLimit == 0.0 && flowErrLimit == 0.0 &&
      flowChangeLimit == 0.0) {
    headErrLimit = 0.005;
  }
...
void GGASolver::findFlowChanges() {
  for (int i = 0; i < linkCount; i++) {
    ...
    if (link->hGrad == 0.0) {
      if (link->isPRV())
        dQ[i] = -xQ[n2] - link->flow;
      if (link->isPSV())
        dQ[i] = xQ[n1] - link->flow;
      continue;
    }
...
double GGASolver::findStepSize(int trials) {
  ...
  if (stepSizing == RELAXATION && oldErrorNorm < ErrorThreshold) {
    ...
    if (errorNorm2 < errorNorm)
      errorNorm = errorNorm2;
    else {
      lamda = 1.0;
      errorNorm = findErrorNorm(lamda);
    }
...
void GGASolver::reportTrial(int trials, double lamda) {
  ...
  if (hydBalance.maxHeadErrLink >= 0) {
    network->msgLog << s_ForLink
                    << network->link(hydBalance.maxHeadErrLink)->name;
  }
  ...
  if (hydBalance.maxFlowErrNode >= 0) {
    network->msgLog << s_AtNode
                    << network->node(hydBalance.maxFlowErrNode)->name;
  }
  ...
  network->msgLog << endl
                  << s_FlowChange
                  << hydBalance.maxFlowChange * network->ucf(Units::FLOW) << " "
                  << network->getUnits(Units::FLOW);
  if (hydBalance.maxFlowChangeLink >= 0) {
    network->msgLog << s_ForLink
                    << network->link(hydBalance.maxFlowChangeLink)->name;
  }
...
bool GGASolver::linksChangedStatus() {
  ...
  for (Link *link : network->links) {
    ...
    if (link->status >= Link::TEMP_CLOSED)
